...
There already exist programs that are modal by default or only need one set of letter bindings (tui programs such as ncmpcpp, vim, evil for emacs (and other less impressive things like vintageous for ST), pentadactyl for firefox, scripts for irssi and weechat, bash and zsh readline vi mode, tmux, and window management with sxhkd or xchainkeys for bindings (though modal bindings aren't as important or even really desirable in the last two cases)).
...
Some problems: Occasionally the video will quit in the middle (not a fault of the script). Sometimes you can't get a direct link (this isn't the case for anything I watch really).
...
- The same physical keys can be used in each context (reducing the total number of needed keys) whereas if super is being used for bspc, it cannot also be used for tmux
Disadvantages:
...
## Make Gifs in MPV
I thought it would be nice to set up bindings within mpv to create gifs. I made a basic script to mark a start time for the gif with o and the end time with O. This requires gifmaker.pl and an actual video file to be playing (with no spaces in the name). I was considering creating a script to use ffmpeg to record fullscreen (with slight cut offs determined with mpv properties) in order to make a gif of any video playing in mpv, but I don't know how ffmpeg would be stopped when not running from a terminal. Sometime I will test it.
